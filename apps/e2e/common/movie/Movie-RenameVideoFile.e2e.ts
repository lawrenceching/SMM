...
describe('Movie - Rename Video File', () => {
    ...
    it('renames the video file and its associated files (subtitles, etc.) via context menu', async function () {
        ...
        console.log('Created movie folder with video + associated files:', movieFolderPath)

        await delay(3 * 1000)

        // Wait for folder to appear in sidebar and select it
        ...
        expect(await fileExistsViaBrowser(joinPlatformPath(movieFolderPath, RENAMED_FILE_NAME))).toBe(true)
