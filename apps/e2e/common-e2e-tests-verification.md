...
| httpproxy | 4 | ✓ all `local, Electron, HarmonyOS, Docker` |
...
All **51 non-manual** specs have correct `@supports` including Docker. No verified non-manual spec is missing Docker in `@supports`.
...
### httpproxy (4) — `@supports local, Electron, HarmonyOS, Docker`
