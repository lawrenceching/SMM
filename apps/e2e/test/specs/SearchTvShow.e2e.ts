...
describe('Search TV Show', () => {
    ...
    it('Search TV Show', async function() {
        ...
        await browser.waitUntil(
            ...
            { timeout: 10000, timeoutMsg: 'ImmersiveSearchbox should be empty for unknown folder' }
        )

        await immersiveInput.click()
        await immersiveInput.setValue('古见同学')
        ...
        const expectedTitle = '古见同学有交流障碍症'
        const resultItem = await $(`//div[@data-testid='tmdb-search-result-item']//h3[contains(text(),'古见同学有交流障碍症')]`)
