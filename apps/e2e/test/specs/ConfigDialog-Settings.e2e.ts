...
describe('Config Dialog Settings', () => {
    ...
    describe('General Settings - Prefer Media Language', () => {
        it('should change and persist prefer media language setting', async function() {
            ...
            let initialCode: '__unset__' | 'zh-CN' | 'en-US' | 'ja-JP' = '__unset__'
            let changed = false
            try {
                // Step 1: Open config dialog
                console.log('Opening config dialog...')
                ...
                // Read initial selected label and choose a deterministic alternate value
                const initialSelected = await ConfigDialog.getSelectedPreferMediaLanguage()
                initialCode = initialSelected.includes('zh-CN')
                    ? 'zh-CN'
                ...
                // Step 2: Change preferMediaLanguage
                ...
                // Step 4: Refresh and reopen dialog
                console.log('Reloading page...')
                await reloadAndWaitForReady()
                await Menu.openConfigDialog()
                await ConfigDialog.waitForDisplayed()

                // Step 5: Verify selected value persisted
                ...
                console.log(`Reverting preferMediaLanguage to: ${initialCode}`)
                if (!await ConfigDialog.isDisplayed()) {
                    await Menu.openConfigDialog()
                    await ConfigDialog.waitForDisplayed()
                }
                await ConfigDialog.setPreferMediaLanguage(initialCode)
                await saveAndCloseDialog()
            }
        })
    })
