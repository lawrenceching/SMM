 ...
 * `@smm/core/ai-tool/systemPrompt` so that the backend (`ChatTask.ts`)
 * and the frontend (`ReverseProxyChatTransport`) can import it from
 * the same source. This module is kept as a re-export shim for
 * legacy import sites.
 *
 * New code should import directly from `@smm/core/ai-tool/systemPrompt`.
 */
 ...
 * `SYSTEM_PROMPT` directly from `@smm/core/ai-tool/systemPrompt`.
 */
