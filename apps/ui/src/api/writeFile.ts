 ...
 * @param mode The write mode (default: 'overwrite')
 * @param traceId Optional trace ID for request tracking (full trace ID string with event name, e.g., "AiSettings-1294")
 */
async function writeFile(path: string, content: string, mode?: 'overwrite' | 'create', traceId?: string): Promise<void> {
  // TODO: check if caller pass traceId as mode which break the function
  if (traceId) {
    console.log(`[${traceId}] writeFile: Writing to ${path} (mode=${mode ?? 'overwrite'})`);
  }
