...
- **ffmpeg / ffprobe**  
  - 来源（2026-08-09 起）：按平台从官方渠道直接下载——linux/win→BtbN FFmpeg-Builds（`FFMPEG_VERSION` 8.1），mac-arm64→osxexperts.net，在 `ffmpeg.Dockerfile` 的 builder 阶段下载并复制到 `/output/bin/ffmpeg/`。  
