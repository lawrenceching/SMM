import { z } from 'zod';
import { getTvShow } from '@/route/Tmdb';
...
/**
 * Get TV show details from TMDB by TMDB ID, including all seasons and episodes.
 */
async function handleTmdbGetTvShow(params: TmdbGetTvShowParams): Promise<ReturnType<typeof createSuccessResponse> | ReturnType<typeof createErrorResponse>> {
...
export const getTool = async function (clientId?: string): Promise<ToolDefinition> {
  const description = await getLocalizedToolDescription('tmdb-get-tv-show');

  return {
    toolName: 'tmdb-get-tv-show',
    description: description,
    inputSchema: z.object({
      id: z.number().describe('TMDB TV show ID (positive integer)'),
    ...
    outputSchema: z.object({
      data: z.any().describe('TV show details from TMDB including seasons and episodes'),
    }),
    execute: async (args: TmdbGetTvShowParams) => {
      return handleTmdbGetTvShow(args);
    },
  };
}
 ...
 */
export async function tmdbGetTvShowAgentTool(clientId: string) {
 ...
 */
export async function tmdbGetTvShowMcpTool() {
  return getTool();
}
