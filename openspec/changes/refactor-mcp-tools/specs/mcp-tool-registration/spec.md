...
- **AND** the input schema SHALL follow JSON Schema format with `type`, `properties`, and `required` fields
- **AND** each parameter SHALL include a `type` and `description`
...
- **THEN** the responseMcpToolResponse SHALL conform to `` interface:
