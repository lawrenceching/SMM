...
- **WHEN** a user activates the **Transcribe** item under the **Subtitle** menu in **`MusicHeaderV2`** and at least one displayed music track has a resolvable local file **`path`**
...
- **WHEN** a user activates the **Transcribe** item under the **Subtitle** menu in **`MusicHeaderV2`** while **`MusicFileTable`** has one or more selected tracks that each have a resolvable **`path`**
...
- **WHEN** a user activates the **Transcribe** item under the **Subtitle** menu in **`MusicHeaderV2`** while no tracks are selected in **`MusicFileTable`** multi-select state (or none of the selected tracks have a **`path`**)
...
- **THEN** the **Transcribe** item under the music panel header **Subtitle** menu is not actionable for transcription (disabled or equivalent)
...
- **THEN** the **Transcribe** item under the music panel header **Subtitle** menu remains actionable for transcription when eligible tracks exist

#### Scenario: No eligible tracks prevents opening dialog

- **WHEN** a user activates the **Transcribe** item under the **Subtitle** menu in **`MusicHeaderV2`** and no displayed track has a resolvable **`path`**
...
The system SHALL open **`TranscribeDialog`** from the **`MusicFileTable`** row context menu's **Subtitle > Transcribe** submenu action and SHALL reset table multi-select so only the context-menu target track is selected inside the dialog.

#### Scenario: Context menu opens dialog with single track checked

- **WHEN** a user invokes **Subtitle > Transcribe** from the context menu on a track that has a resolvable **`path`** while **either** VideoCaptioner is available **or** Tencent ASR transcription is enabled for the application
...
- **THEN** the **Transcribe** submenu item is not actionable for transcription (disabled or equivalent)
