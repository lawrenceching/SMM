...
Define transcribe entry points for **MusicPanel**: **`MusicHeaderV2`** and **`MusicFileTable`** context menu opening **`TranscribeDialog`**, row mapping from music files, default selection rules, and VideoCaptioner gating.
...
- **AND** the dialog lists one row per such track with a **`path`** suitable for **VideoCaptioner** transcription
...
- **WHEN** VideoCaptioner discovery indicates unavailable for transcription
...
- **WHEN** a user invokes **Transcribe** from the context menu on a track that has a resolvable **`path`** while VideoCaptioner is available
...
#### Scenario: Context menu transcribe disabled when VideoCaptioner unavailable
...
The system SHALL populate **`TranscribeDialog`** rows from **`MusicPanel`** music file data such that each transcribe execution receives a resolvable file path for **VideoCaptioner**, using stable row identifiers consistent with TV and movie **`TranscribeDialog`** rows.
