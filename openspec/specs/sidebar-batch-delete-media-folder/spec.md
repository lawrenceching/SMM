# sidebar-batch-delete-media-folder Specification

## Purpose
Defines batch deletion of media folders from the sidebar: DELETE key removes selected folder(s), context menu Delete removes all selected folders when multiple are selected, reusing existing per-folder delete logic (remove metadata, update user config).
