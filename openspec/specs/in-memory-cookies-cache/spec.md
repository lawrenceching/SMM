...
In-memory cookie configuration cache keyed by hostname for the download video dialog.

## Purpose

Pre-fill cookie configuration fields when the user enters a URL whose domain was previously configured in the same session, eliminating re-configuration for repeated downloads from the same site.
