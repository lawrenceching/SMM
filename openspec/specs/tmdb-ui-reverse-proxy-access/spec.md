...
Define how the UI sends TMDB requests through the CLI-managed reverse proxy, including upstream selection from `UserConfig.tmdb.host`, attaching `Authorization` from the configured API key, and preserving method/path/query/body without an `/tmdb` URL prefix.
