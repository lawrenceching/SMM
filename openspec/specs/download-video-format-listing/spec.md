...
Define how yt-dlp format presets are resolved to selector expressions, and how per-video format codes from `yt-dlp -J` JSON output populate the format code dropdown via `VideoMetadata.formats`.
## Requirements
### Requirement: Format quality presets use yt-dlp selector expressions

The UI SHALL offer a fixed set of format **presets** (automatic, best quality, 1080p cap, 720p cap, audio only). In addition, after successful `yt-dlp -J`, the UI SHALL offer **format code** mode populated from `VideoMetadata.formats`.

Each non-default preset SHALL map to a yt-dlp **format selector string** (not a per-video numeric `format_id`). Each format code selection SHALL map to the format ID(s) from the `VideoMetadata.formats` array.
...
The `yt-dlp -J` call SHALL be triggered only when the user clicks the "Go" button or presses Enter in the URL input field. URL blur SHALL NOT trigger format listing.

#### Scenario: User clicks Go

- **WHEN** the user clicks the "Go" button
- **THEN** `yt-dlp -J` SHALL be called with the current URL and cookie/JS-runtime settings

#### Scenario: User presses Enter in URL field

- **WHEN** the user presses Enter while the URL input is focused
- **THEN** `yt-dlp -J` SHALL be called (same as clicking Go)
...
- **THEN** `yt-dlp -J` SHALL NOT be called

### Requirement: Loading state shown during format listing

While `yt-dlp -J` is executing, the "Go" button SHALL display a spinner icon in place of the "Go" text, indicating loading state. The button SHALL be disabled during this time.

#### Scenario: Loading spinner during format fetch

- **WHEN** the user clicks "Go" and `yt-dlp -J` begins executing
...
- **WHEN** `yt-dlp -J` completes (success or error)
...
On successful `yt-dlp -J`, the `VideoMetadata.formats` array SHALL populate the format code dropdown, grouped into audio only, video only, and audio+video categories.

#### Scenario: Format codes populated from listing

- **WHEN** `yt-dlp -J` returns format data
- **THEN** the format code dropdown SHALL be populated with all entries from `VideoMetadata.formats`
- **AND** entries SHALL be grouped by category

### Requirement: Error handling applies to both list-formats and download phases

Error detection SHALL apply to both `yt-dlp -J` output and download execution output. The same error patterns (cookie expiry, format unavailable, unknown error) SHALL be checked in both phases.

#### Scenario: Cookie expiry during list-formats

- **WHEN** `yt-dlp -J` stderr contains "The provided YouTube account cookies are no longer valid"
...
- **WHEN** either `yt-dlp -J` or download fails with an unrecognized error pattern
